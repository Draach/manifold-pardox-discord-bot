...
internal class SlashCommandHandler
{
    ...
    private async Task EnrollForWaitlist(SocketSlashCommand command)
    {
        try
        {
            ...
            DocumentReference userDocRef = firestoreDb.Collection("waitlist").Document(socketUser.Username);
            DocumentSnapshot userDocSnapshot = await userDocRef.GetSnapshotAsync();
            if (!userDocSnapshot.Exists)
            {
                await AddUserToWaitlist(command, socketUser, userDocRef);
            }
            else
            {
    ...
    private static async Task AddUserToWaitlist(SocketSlashCommand command, SocketUser socketUser, DocumentReference docRef)
    {
    ...
    private async Task CheckWaitlist(SocketSlashCommand command)
    {
        try
        {
            ...
            if (hasPrivilegeRole)
            {
                ...
                await SendWaitlistAsync(command, waitlistSnapshots);
            }
            else
            {
    ...
    private static async Task SendWaitlistAsync(SocketSlashCommand command, QuerySnapshot waitlistSnapshots)
    {
